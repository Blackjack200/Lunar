	...
	public function processClient(DataPacket $packet) : void {
		if ($packet instanceof MovePlayerPacket) {
			...
			if ($info->moveDelta->lengthSquared() > 0.009) {
				...
				if ($info->onGround !== $onGround) {
					$player->sendMessage('diff');
				}
				$player->sendPopup('pm=' . Boolean::btos($onGround) . ' on=' . Boolean::btos($info->onGround) . ' check=' . Boolean::btos($info->checkFly) . ' tick=' . $info->inAirTick);
				*/
			}
		}
	}
